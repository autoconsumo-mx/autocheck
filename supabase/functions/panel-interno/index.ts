...
// Cuentas que sí verificaron su correo (recibieron y usaron un código OTP real), aunque nunca
// hayan enviado un autocheck — para que el panel las muestre con "0 de N" en vez de
// desaparecer. Quien solo pidió el código pero nunca lo verificó (confirmed_at vacío) no cuenta.
async function listarUsuariosVerificados(supabaseAdmin: ReturnType<typeof createClient>) {
  const todos: Array<{ email: string; created_at: string }> = [];
  let page = 1;
  while (true) {
    const { data, error } = await supabaseAdmin.auth.admin.listUsers({ page, perPage: 200 });
    if (error) throw error;
    for (const u of data.users) if (u.email && u.confirmed_at) todos.push({ email: u.email, created_at: u.created_at });
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const [{ data: leads, error: leadsErr }, { data: membresias }, { data: pagos }, usuarios] = await Promise.all([
      supabaseAdmin
      ...
      listarUsuariosVerificados(supabaseAdmin),
    ]);
    if (leadsErr) throw new Error("No se pudieron leer los autochecks: " + leadsErr.message);

    const altaPorCorreo = new Map<string, string>();
    for (const u of usuarios) altaPorCorreo.set(u.email.trim().toLowerCase(), u.created_at);
    ...
    // Agrupa cada autocheck por el correo (en minúsculas) de su suscriptor. Incluye también a
    // quien se registró pero nunca envió ninguno (queda con arreglo vacío, "0 de N").
    const porCorreo = new Map<string, typeof leads>();
    for (const correo of altaPorCorreo.keys()) porCorreo.set(correo, []);
    ...
    let con0 = 0, con1 = 0, con2 = 0, con3Mas = 0;
    ...
    for (const [correo, filas] of porCorreo) {
      ...
      const ultimo = n > 0 ? filas![filas!.length - 1] : null;
      const primero = n > 0 ? filas![0] : null;
      ...
      miembros.push({
        ...
        correo,
        alta: primero?.created_at || altaPorCorreo.get(correo) || null,
