...
```
**Pendientes menores encontrados en esta prueba (ambos dashboard-only, no requieren código):**
1. El correo de "Confirm signup" sigue sin la plantilla de marca (body Y subject en inglés genérico de Supabase) — falta que Alfredo pegue el HTML de arriba. Además **se fue a spam** en Gmail — probablemente por ser un correo genérico de un solo link desde un dominio con poca reputación de envío todavía; vale la pena que Alfredo revise en Resend → Domains que SPF/DKIM/DMARC de `autoconsumo.mx` estén verificados (no hay ningún tool de Resend conectado para chequear esto por MCP).
2. El correo de OTP ("Magic link or OTP", ya con el body correcto) tiene el **Subject todavía en inglés** ("Your sign-in link") — nunca se editó ese campo. Se cambia en Authentication → Email Templates → Magic Link → Subject.
...
4. ~~Decidir Opción A vs B para la plantilla "Confirm sign up"~~ — ✅ decidido, codificado y **verificado en vivo** en sesión 8 (21-sep-2026): Opción A con auto-envío del OTP al confirmar, probado de punta a punta con un alta real. Solo faltan dos ajustes de dashboard (plantilla+subject de "Confirm signup", subject de "Magic Link") y revisar por qué el correo de confirmación cae en spam — ver detalle en "Verificación por OTP" arriba.
